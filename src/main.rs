...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        Commands::Check(cli_args) => {
            ...
            let model_response = scanner_manager.run_scan(&prompt_manager.system_prompt_scan, &prompt_manager.user_prompt, &config.model.expect("A model must be provided"), config.provider.expect("A provider must be provided."), config.production_mode).await?;
