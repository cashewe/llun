...
class Rules(
    BaseModel
):  # why does pydantic RootModel exist? seems to add nothing but complexity?
    rules: list[Rule]
...
class RuleRegistry:
    ...
    def get_rules(self, rule_codes: list[str] | None) -> Rules:
        ...
        return Rules(
            rules=[
                self.rules_dict[code] for code in rule_codes if code in self.rules_dict
            ]
        )
